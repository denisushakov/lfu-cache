...
export const Rectangle: FC<TRectangle> = ({
  ...
  const getBadgeColor = (freq: number | undefined) => {
    if (freq === undefined) return COLORS.FREQUENCY_BADGE_BG;
  ...
  return (
    <>
      ...
      {frequency !== undefined && !isNaN(frequency) ? (
        <g>
          <circle
            cx={xPostion + 12}
            cy={yPosition + 12}
            r={10}
            fill={getBadgeColor(frequency)}
            opacity={0.9}
          />
          <text
            textAnchor="middle"
