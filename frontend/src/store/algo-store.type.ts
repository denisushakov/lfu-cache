...
export type TMapItem = {
  key: number;
 data: { value: number; address: string; frequency: number };
};
...
export type TMapState = {
  map: Map<TMapItem["key"], Omit<TMapItem["data"], "address"> & { address: string }>;
};
type TMapApi = {
  updateMap: () => void;
};
...
type TDiagramApi = {
  addNewNode: () => void;
  incrementFrequency: (key: TSvgNodeItem["key"]) => void;
